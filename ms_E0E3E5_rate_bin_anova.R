...
#### load data ####
analysisName = "E0E3E5_msrate_-500to0ms_bin500ms_N30"
...
lmmResults <- ddply(data, 'bin_start', function(x){
  ## LMM ##
  lmm <- lmer(msrate ~ cue * experiment + (1|subject), data=x)
  print(x$bin_start[1])
  print(summary(lmm))
  #save model as R files
  save(lmm, file = paste(path, '/', paste(analysisName,x$bin_start[1],'model',sep='_'), ".rda", sep=""))
  #store resulting statistics and p value in a data frame
  data.frame(beta_cueneutral = summary(lmm)$coefficients[2,1], beta_cuet2 = summary(lmm)$coefficients[3,1], pval_cue = Anova(lmm)[1,3])
})
...
for (i in 1:length(binStarts)){
  listOfBootstrappedModels <- c(listOfBootstrappedModels, paste(analysisName,binStarts[i],'model',sep='_'))
}
