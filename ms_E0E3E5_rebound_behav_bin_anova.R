# ms_E0E3E5_rebound_behav_bin_anova.R
...
#### load data ####
analysisName = "E5_msRebound2LatencyVBehavBin_bin200ms_N9_c2500ms"
data <- read.table(paste(analysisName,".txt",sep=""), header = T, sep=" ") 

data$subject <- factor(data$subject)
data$experiment <- factor(data$experiment)
data$target <- factor(data$target)
data$bin_or_mean <- factor(data$bin_or_mean)

# visualize data
plot(data$target, data$acc)
plot(data$bin_start, data$acc)
...
lmmResults <- ddply(data, 'target', function(y){
  ddply(y, 'bin_start', function(x){
  ## LMM ##
  #lmm <- lmer(acc ~ bin_or_mean * experiment + (1|subject), data=x)
  lmm <- lmer(acc ~ bin_or_mean + (1|subject), data=x)
  print(x$bin_start[1])
  print(summary(lmm))
  #save model as R files
  save(lmm, file = paste(path, '/', paste(analysisName,x$target[1],x$bin_start[1],'model',sep='_'), ".rda", sep=""))
  #store resulting statistics and p value in a data frame
  data.frame(beta_bin_or_mean = summary(lmm)$coefficients[2,1], pval_bin_or_mean = Anova(lmm)[1,3])
})})
